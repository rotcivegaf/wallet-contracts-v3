 ...
 */
interface ISapient {
    function isValidSapientSignature(Payload.Decoded calldata _payload, bytes calldata _signature)
        external
        view
        returns (bytes32);
}
...
interface ISapientCompact {
    function isValidSapientSignatureCompact(bytes32 _digest, bytes calldata _signature)
        external
        view
        returns (bytes32);
}
