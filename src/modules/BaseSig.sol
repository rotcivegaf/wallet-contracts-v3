...
contract BaseSig {
    ...
    function recover(Payload.Decoded memory _payload, bytes calldata _signature)
        internal
        view
    ...
    function recoverBranch(Payload.Decoded memory _payload, bytes32 _subdigest, bytes calldata _signature)
        internal
        view
            ...
            while (rindex < _signature.length) {
                ...
                if (flag == FLAG_SIGNATURE_SAPIENT) {
                    ...
                    bytes32 sapientImageHash =
                        ISapient(addr).isValidSapientSignature(_payload, _signature[rindex:nrindex]);
